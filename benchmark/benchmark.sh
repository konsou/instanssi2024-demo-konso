...
echo "Load averages before benchmark: $load_avg" | tee -a "${OUTPUT_FILENAME}"
(cd "${PROJECT_ROOT}" && mvn exec:java -Dexec.args="--benchmark --runtime $RUNTIME_SECONDS --fps-cap $FPS_CAP") 2>&1 | tee -a "${OUTPUT_FILENAME}"
# Check the exit status of the java command, not the tee
if [ "${PIPESTATUS[0]}" -ne 0 ]; then
